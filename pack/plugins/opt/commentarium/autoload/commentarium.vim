...
if get(g:, 'autoloaded_commentarium') || !get(g:, 'commentarium_enabled')
  finish
endif
...
export def DoComment(line: number, col: number)
  ...
  elseif index(COMMENT_VIM, &filetype) >= 0
    ...
    EchoErrorMsg($"Error: commenting with the filetype '{&filetype}' is not supported")
  endif
enddef
...
export def UndoComment(line: number, col: number): void
  ...
  elseif index(COMMENT_VIM, &filetype) >= 0
    ...
    EchoErrorMsg($"Error: uncommenting with the filetype '{&filetype}' is not supported")
  endif
enddef
