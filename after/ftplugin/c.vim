...
if get(g:, "lsp_enabled")
  ...
  && reg_recording() == ''
    ...
    # trigger for '.'
    inoremap <buffer> <nowait> <silent> <expr> . (col('.') > 1 && getline('.')[col('.') - 2] =~ '\k\<Bar>)\<Bar>]\<Bar>}') ? ".\<C-x>\<C-o>" : "."
    # trigger for '->'
    inoremap <buffer> <nowait> <silent> <expr> > (col('.') > 2 && getline('.')[col('.') - 2] == '-' && getline('.')[col('.') - 3] =~ '\k\<Bar>)\<Bar>]') ? ">\<C-x>\<C-o>" : ">"
  endif
endif
